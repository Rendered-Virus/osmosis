...
public class DamageFlash : MonoBehaviour
{
    ...
    protected virtual void Start()
    {
        ...
        foreach (Renderer renderer in meshRenderers)
        {
            ...
            for (int i = 0; i < renderer.materials.Length; i++)
            {
                ...
                {
                    colors.Add(_flashColor); // Dummy color for materials without _Color property
                }
            }
    ...
    {
        ChangeMaterialsToColor(_flashColor * 2); // 255, 255, 255
        yield return new WaitForSeconds(0.2f);

        revertCoroutine = StartCoroutine(RevertMaterialsSmoothly(flashDuration));
    }
        ...
        while (elapsedTime < duration)
        {
            ...
            for (int i = 0; i < meshRenderers.Length; i++)
            {
                for (int j = 0; j < meshRenderers[i].materials.Length; j++)
                {
                    ...
                    {
                        meshRenderers[i].materials[j].color = Color.Lerp(_flashColor, originalColors[i][j], t);
                    }
                }
            }
